...
def convert_examples_to_features(item):
    ...
    #extract data flow
    code_tokens,dfg=extract_dataflow(js['method_content'],parser,args.lang)
    ...
    method_id = ['method_path']+'#'+js['method_name']
    
    return InputFeatures(code_tokens,code_ids,position_idx,dfg_to_code,dfg_to_dfg,nl_tokens,nl_ids,method_id)
...
def train(args, model, tokenizer,pool):
    ...
    for idx in range(args.num_train_epochs): 
        ...
        if results['eval_mrr']>best_mrr:
            ...
            output_dir = os.path.join(output_dir, '{}'.format('anki_model_docstring_code_with_comment.bin'))
...
def evaluate(args, model, tokenizer,file_name,pool, eval_when_training=False):
    ...
    for example in query_dataset.examples:
        nl_methods.append(example.method_id)
        
    for example in code_dataset.examples:
        code_methods.append(example.method_id)

    ranks=[]
    for url, sort_id in zip(nl_methods,sort_ids):
...
def main():
    ...
    results = {}
    if args.do_eval:
        checkpoint_prefix = 'checkpoint-best-mrr/anki_model_docstring_code_with_comment.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
